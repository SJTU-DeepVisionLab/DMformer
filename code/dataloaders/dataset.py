...
class Synapse_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            data_path = self._base_dir + "/data/train_npz/{}.npz".format(slice_name)
        ...
        #    image, label = data['image'], data['label']
            ...
            #filepath = self._base_dir + "/data/test_vol_h5/{}.npy.h5".format(vol_name)
            #data = h5py.File(filepath)
            #image, label = data['image'][:], data['label'][:]
        #数据泄漏
        else:
            vol_name = self.sample_list[idx]#.strip('\n')
            filepath = self._base_dir + "/data/val_set/{}.h5".format(vol_name)
            data = h5py.File(filepath)
            image, label = data['image'][:], data['label'][:]
