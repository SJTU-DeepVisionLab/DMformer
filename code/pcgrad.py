...
class PCGrad():
    def __init__(self, optimizer, reduction='sum', calculate=False):
        self._optim, self._reduction, self._calculate = optimizer, reduction, calculate
        return
    ...
    def pc_backward(self, objectives):
        ...
        if self._calculate:
            pc_grad, angle = self._project_conflicting(grads, has_grads)
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        for g_i in pc_grad:
            ...
            for g_j in grads:
                ...
                if g_i_g_j < 0:
                    angles+=1
                    #angles.append(torch.acos(torch.clamp(torch.dot(g_i, g_j)/(g_j.norm()*g_i.norm()), -1.0, 1.0))* (180.0 / torch.pi))
        ...
        merged_grad[~shared] = torch.stack([g[~shared]
                                            for g in pc_grad]).sum(dim=0)
        if self._calculate:
            return merged_grad, angles / (angles_all + 1e-8)
        else:
            return merged_grad
