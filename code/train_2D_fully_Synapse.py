...
from dataloaders.dataset import (BaseDataSets, RandomGenerator, Synapse_dataset,
...
parser.add_argument('--exp', type=str,
                    default='NIPS23/Synapse_tmi_only_0.5+0.5_1337_0.01/', help='experiment_name')
...
parser.add_argument('--mask_thershold', type=float,
                    default=0.01, help='mask_ratio')

parser.add_argument('--mask_class_ratio', type=float,
                    default=0.0, help='mask_ratio')
...
parser.add_argument('--adjustment_rate_inter', type=float,
                    default=0.00005, help='adjustment_rate')
...
def update_confidence(args, outputs_soft1_intra, input_mask_intra, outputs_soft1_inter, input_mask_inter, outputs_soft2, current_average_confidence_intra, increase_intra, average_confidence_intra, current_average_confidence_inter, average_confidence_inter):
    ...
    all_existing_class_inter = torch.unique(torch.argmax(outputs_soft1_intra[args.labeled_bs:], dim=1))#* (1-input_mask_inter[args.labeled_bs:].squeeze(1).long()))
    ...
    for class_i in range(args.num_classes):
        ...
        if class_i in all_existing_class:
            ...
            current_average_confidence_intra[class_i] = (torch.sum((outputs_soft1_intra[args.labeled_bs:][:, class_i, :, :])*(prediction_class_intra==class_i))) / (torch.sum((prediction_class_intra==class_i)) + 1e-5)
...
def adjust_parameters_by_confidence2(args, confidence_increase_intra,increase_intra_num, confidence_increase_inter, increase_inter_num, mask_threshold, mask_class_ratio, alpfa):
    adjustment_rate_intra = args.adjustment_rate_intra*np.round(np.tanh(increase_intra_num/(args.adjustment_rate_intra)),decimals=6)#*(1.0 - alpfa)**2#**0.5
    adjustment_rate_inter = args.adjustment_rate_inter*np.round(np.tanh(increase_inter_num/(args.adjustment_rate_inter)),decimals=6)#*(1.0 - alpfa)**2#*10#**0.5

    mask_threshold  = np.round(np.clip(mask_threshold + adjustment_rate_intra, args.mask_thershold, 0.75),decimals=6)
            

    mask_class_ratio = np.round(np.clip(mask_class_ratio + adjustment_rate_inter, args.mask_class_ratio, 0.75),decimals=6)
...
def train(args, snapshot_path):
    ...
    db_train = Synapse_dataset(base_dir=args.root_path, split="train", num=None, transform=transforms.Compose([
    ...
    average_confidence_inter = 0.25 * np.ones(args.num_classes)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if alpfa >= 0.1:
                masked_volume_batch_intra, masked_volume_batch_inter, input_mask_intra, input_mask_inter, use_inter = mask_gen.mask_image_two_inter_intra_adaptive_flexmatch_spe_lianshi_conf(volume_batch[args.labeled_bs:], outputs_soft2[args.labeled_bs:], mask_thershold, mask_class_ratio, iter_num,average_confidence_inter, True,False, args.num_classes)    
            else:
                masked_volume_batch_intra, masked_volume_batch_inter, input_mask_intra, input_mask_inter, use_inter = mask_gen.mask_image_two_inter_intra_adaptive_flexmatch_spe_lianshi_conf(volume_batch[args.labeled_bs:], outputs_soft2[args.labeled_bs:], mask_thershold, mask_class_ratio, iter_num,average_confidence_inter, True,True, args.num_classes)
            ...
            loss1 = 0.4 * ce_loss(outputs1_intra[:args.labeled_bs], label_batch[:args.labeled_bs].long()) + 0.6* dice_loss(
            ...
            pseudo_supervision1_intra = dice_loss2(
            ...
            pseudo_supervision1_inter = dice_loss2(
                outputs_soft1_inter[args.labeled_bs:], pseudo_outputs2.unsqueeze(1), 1- input_mask_intra[args.labeled_bs:].squeeze(1) + input_mask_inter[args.labeled_bs:].squeeze(1))
            #if use_inter:
            model1_loss = loss1 + consistency_weight * pseudo_supervision1_intra + consistency_weight * pseudo_supervision1_inter
            model2_loss = loss2
            ...
            optimizer = PCGrad(optimizer1, 'sum', True) 
            unsup_loss = [model1_loss_intra, model1_loss_inter] 
            assert len(unsup_loss) == 2
            angle = optimizer.pc_backward(unsup_loss) 
            optimizer.step()  
            ...
            if iter_num > 0 and iter_num % 100 == 0:
                ...
                logging.info('mask_thre %f %f %f %f %f %f %f %f %f : mask_cls %f '% (mask_thershold[0].item(), mask_thershold[1].item(), mask_thershold[2].item(), mask_thershold[3].item(), mask_thershold[4].item(), mask_thershold[5].item(),  mask_thershold[6].item(), mask_thershold[7].item(), mask_thershold[8].item(), mask_class_ratio.item()))
                logging.info('avi %f %f %f %f %f %f %f %f %f : aci %f' % (average_confidence_intra[0].item(), average_confidence_intra[1].item(), average_confidence_intra[2].item(), average_confidence_intra[3].item(), average_confidence_intra[4].item(), average_confidence_intra[5].item(), average_confidence_intra[6].item(), average_confidence_intra[7].item(), average_confidence_intra[8].item(), np.mean(average_confidence_inter).item()))
            if iter_num % 50 == 0:
            ...
            if iter_num >= 0 and iter_num % 100 == 0:
                ...
                plt.title('Angle Between Two Vectors During Training')
                plt.savefig(os.path.join(snapshot_path, 'angle.png'))
                plt.close()
                with open("angles_w_pcgrad.txt", "w") as file:
                    for angle in angles:
                        file.write(f"{angle}\n")
            if iter_num >= 20000 and iter_num % 2000 == 0:
                model1.eval()
                metric_list = 0.0
                ...
                writer.add_scalar('info/mask_thershold1',
                                      mask_thershold[0], iter_num)
                writer.add_scalar('info/mask_thershold2',
                                      mask_thershold[1], iter_num)
                writer.add_scalar('info/mask_thershold3',
                                      mask_thershold[2], iter_num)
                writer.add_scalar('info/mask_thershold4',
                                      mask_thershold[3], iter_num)
                writer.add_scalar('info/mask_thershold5',
                                      mask_thershold[4], iter_num)
                writer.add_scalar('info/mask_thershold6',
                                      mask_thershold[5], iter_num)
                writer.add_scalar('info/mask_thershold7',
                                      mask_thershold[6], iter_num)
                writer.add_scalar('info/mask_thershold8',
                                      mask_thershold[7], iter_num)
                writer.add_scalar('info/mask_thershold9',
                                      mask_thershold[8], iter_num)
                writer.add_scalar('info/mask_class_ratio',
                                      mask_class_ratio, iter_num)
                ...
                logging.info(
                    'iteration %d : model1_mean_dice : %f model1_mean_hd95 : %f model1_mean_asd : %f' % (iter_num, performance1, mean_hd951, mean_asd1))
                model1.train()
                ...
                logging.info(
                    'iteration %d : model2_mean_dice : %f model2_mean_hd95 : %f model2_mean_asd : %f' % (iter_num, performance2, mean_hd952, mean_asd2))
                model2.train()
